...
const store = (set, get) => ({
    ...
    calculateTotal:()=>{
       ...
       const total= currentCart.reduce((sum,product)=>sum+=(product.qty*product.price),0)
        set(state=>({cart:get().cart,total}))
    },
    removeFromCart:(attributes,id)=>{
        const currentCart=[...get().cart]
        attributes.id=id
        const product=currentCart[currentCart.findIndex(elem=> elem.id==attributes.id)]
    ...
    addToCart: (attributes, id) => {
        ...
        attributes.id = id
        const check_index=tempCart.findIndex(product=>product.id==id)
        // Push item to cart if not already Present In Cart
        if (check_index!==-1) {
            const productPresent = tempCart[check_index]
            productPresent.qty++;
            productPresent.subTotal=parseInt(productPresent.qty)*parseInt(productPresent.price)
...
const useCartStore = create(persist(store,{
    name:'product-cart',
